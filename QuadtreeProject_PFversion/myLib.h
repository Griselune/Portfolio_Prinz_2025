...
namespace ML
{
	...
	{
	public:
		//int x, y, w, h;		//2025.08.03 プリンス
		float x, y, w, h;		//2025.08.03 プリンス int->float
		Box2D( )
		:x(0), y(0), w(0), h(0)
		{
		}
		//Box2D(int x_, int y_, int w_, int h_)　	//2025.08.03 プリンス
		Box2D(float x_, float y_, float w_, float h_)		//2025.08.03 プリンス int->float
		:x(x_), y(y_), w(w_), h(h_)
		{
		}
