...
namespace  Enemy001
{
	...
	//	this->imgEnemy = DG::Image::Create("./data/image/Characters/SpaceShooterAssetPack_Miscellaneous.png");
		...
		se::LoadFile("EnemyShot", "./data/Sounds/Weapons/EnemyShot.wav");
		se::SetVolume("EnemyShot", 60);
	...
	bool  Object::Initialize()
	{
		...
		this->shotDamageMax = 5;
		this->shotDamage = this->shotDamageMax;
	...
	void  Object::UpDate()
	{
		...
		if (this->state == State::Normal) {
			...
			if (this->shootingTimer <= 0) {
				auto shot = CannonEnemy::Object::Create(true);
				se::Play("EnemyShot");
				shot->pos = this->pos + ML::Vec2(0.0f, 32.0f);
				shot->speed = 5.0f;
				shot->gravityPull = this->gravityPull;
				shot->damage = this->shotDamage;
	...
	void  Object::Render2D_AF()
	{
		...
		case State::Normal:
			if (debug) {
				...
				this->res->imgEnemy->Rotation(ML::ToRadian(this->rotation), { 32.f, 32.f }); //randomized rotation of the sprite because drifters are empty and dead
