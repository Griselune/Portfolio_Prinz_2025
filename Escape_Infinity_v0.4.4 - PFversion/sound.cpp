...
// 再生位置初期化
//--------------------------------------------------------------------------------------------
void bgm::SetStartPos(const std::string& resname, LONGLONG start_) //modified by Prinz - added start_ for unpausing bgm
{

	if (BgmTable.at(resname).bgm.use == 0) return;
	LONGLONG	start = start_; //LONGLONG	start = 0; 
	BgmTable.at(resname).bgm.pMediaSeeking->SetPositions(&start, AM_SEEKING_AbsolutePositioning,
																											 NULL, AM_SEEKING_NoPositioning);
}
...
//	se.sBuffer->SetFrequency(se.Fre + Fre);
	se.PlayBuffer->pBuffer->SetFrequency(se.Fre + Fre);
}
...
void se::SetVolume(const std::string& resname, int Vol)
{
	...
	volume = (long)(1000.0 * (log10(((double)Vol / 100.0))));
